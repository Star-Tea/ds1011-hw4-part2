...
def save_model(checkpoint_dir, model, best):
    ...
    if best:
        ...
        checkpoint_path = os.path.join(checkpoint_dir, f"last_model.pt")
...
def load_model_from_checkpoint(args, best):
    ...
    checkpoint_path = os.path.join(
        args.checkpoint_dir, 
        f"best_model.pt" if best else f"last_model.pt"
    )
    
    model = initialize_model(args) # First initialize a new model
    model.load_state_dict(torch.load(checkpoint_path)) # Then load states
